Carboncoin Core version 0.11.0 is now available from:

  <https://carboncoin.org/bin/carboncoin-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer CARBON/kB fee to be
...
For example, add the following to `carboncoin.conf`:
...
related to the blockchain in the carboncoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Carboncoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Carboncoin Core will 
...
For an example script see [carboncoin-submittx](https://github.com/laanwj/carboncoin-submittx).
...
- #5334 `cf87536` libcarboncoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new carboncoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only CarboncoinAmountField
- #5753 `0f44672` Add carboncoin logo to about screen
...
- #5809 `46bfbe7` Add carboncoin-cli man page
...
- #5849 `41259ca` contrib/init/carboncoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Carboncoin-Qt.app to Carboncoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for carboncoin-tx)
