...
const char* CARBONCOIN_REQUEST_MIMETYPE = "application/carboncoin-paymentrequest";
const char* CARBONCOIN_PAYMENTACK_MIMETYPE = "application/carboncoin-paymentack";
const char* CARBONCOIN_PAYMENTACK_CONTENTTYPE = "application/carboncoin-payment";
...
//
// OSX-specific way of handling carboncoin: URIs and
// PaymentRequest mime types
//
bool PaymentServer::eventFilter(QObject *object, QEvent *event)
{
    // clicking on carboncoin: URIs creates FileOpen events on the Mac
    ...
    if (s.startsWith(CARBONCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // carboncoin: URI
    {
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseCarboncoinURI(s, &recipient))
                ...
                emit message(tr("URI handling"),
                    tr("URI can not be parsed! This can be caused by an invalid Carboncoin address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    foreach(const PAIRTYPE(CScript, qint64)& sendingTo, sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Insecure payments to custom carboncoin addresses are not supported
...
void PaymentServer::fetchRequest(const QUrl& url)
{
    ...
    netRequest.setRawHeader("Accept", CARBONCOIN_REQUEST_MIMETYPE);
...
void PaymentServer::fetchPaymentACK(CWallet* wallet, SendCoinsRecipient recipient, QByteArray transaction)
{
    ...
    netRequest.setHeader(QNetworkRequest::ContentTypeHeader, CARBONCOIN_PAYMENTACK_CONTENTTYPE);
    netRequest.setRawHeader("User-Agent", CLIENT_NAME.c_str());
    netRequest.setRawHeader("Accept", CARBONCOIN_PAYMENTACK_MIMETYPE);
